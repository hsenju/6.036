...
def N(X,Mu,Var,d):
    return (1/(np.power((2*np.pi*Var[0]),(d/2))))*np.exp((-1/(2*Var[0]))*np.power(np.linalg.norm(X-Mu),2))

def summation(X,K,Mu,P,Var,d):
    s = 0
    for l in range(K):
        s += P[l]*N(X,Mu[l],Var[l],d)
    return s
...
def Estep(X,K,Mu,P,Var):
    ...
    LL = 0
    for i in range (n):
        s = summation(X[i],K,Mu,P,Var,d)
        innersum = 0
        for j in range (K):
            post[i,j] = P[j]*N(X[i],Mu[j],Var[j],d)/s;
            innersum += P[j]*N(X[i],Mu[j],Var[j],d)
        LL += np.log(innersum)
    return (post,LL)
...
def Mstep(X,K,Mu,P,Var,post):
    ...
    for j in range (K):
        ...
        for t in range (n):
            nsum += post[t,j]
            usum += post[t,j] * X[t]
            vsum += post[t,j] * np.power(np.linalg.norm(X[t]-Mu[j]),2)
        P[j] = nsum/n
        Mu[j] = usum/nsum
        Var[j] = vsum/(nsum*d)
    print "P: " + str(P)
...
(Mu,P,Var,post)  = EMAlgorithm(X, K, Mu, P, Var) 
